...
The `home-routes` handler in the `hipstr.routes.home` namespace
...
The `home-page` function will in turn call the `hipstr.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `hipstr.handler` namespace:
...
Middleware used for development is placed in the `hipstr.dev-middleware` namespace found in
